...
const client = new Client({
    ...
    puppeteer: {
        ...
        args: [
            ...
            '--disable-web-security', '--disable-features=site-per-process',
            '--disable-infobars', '--window-position=0,0', '--ignore-certificate-errors',
            '--ignore-certificate-errors-spki-list', '--disable-gpu', '--disable-webgl'
        ]
    },
});
...
app.post('/api/send', async (req, res, next) => {
    ...
    try {
        ...
        if (!phone || !message) {
            return res.status(400).json({ status: false, message: 'Error', meta: 'Phone and message are required' });
        }
